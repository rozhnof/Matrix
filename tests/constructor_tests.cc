...
TEST(Constructor, Ctor1) {
    ...
    EXPECT_EQ(static_cast<Allocator<int>>(matrix.GetAllocator()).memory_allocated, 0);
}
...
TEST(Constructor, Ctor6) {
    EXPECT_THROW((Matrix<int, AllocCtorThrow<int>>(1, 1, int{})), std::runtime_error);
}
...
TEST(Constructor, Ctor9) {
    ...
    EXPECT_EQ(static_cast<Allocator<int>>(matrix.GetAllocator()).memory_allocated, 25);
}
...
TEST(Constructor, Ctor10) {
    ...
    EXPECT_THROW((Matrix<CtorThrowType, Allocator<CtorThrowType>>(2, 2)), std::runtime_error);
